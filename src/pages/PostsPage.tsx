...
const [token, setToken] = useState<string>(
  ...
  // Estados para formulario POST (Crear tarea)
  const [newTitle, setNewTitle] = useState("");
  const [newDesc, setNewDesc] = useState("");
  const [projectId, setProjectId] = useState("1");

  // Función que construye los headers con el Bearer Token
  const getHeaders = () => {
  ...
  return (
    <Stack spacing={3}>
      ...
      {/* Panel opcional por si quieres cambiar el token en caliente */}
      ...
      {/* Formulario POST para crear tarea */}
      ...
      {!loading && !error && tasks.length === 0 && (
        <Alert severity="info">La API no devolvió tareas o el token expiró.</Alert>
      )}
        ...
        return (
          <>
            {filteredTasks.length === 0 ? (
              <Alert severity="info">No hay tareas que coincidan con la búsqueda.</Alert>
            ) : (
              <>
                <Stack spacing={2}>
